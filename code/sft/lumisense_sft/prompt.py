...
def training_record(record: dict[str, Any], label: dict[str, Any]) -> dict[str, Any]:
    target = label["target"]
    if target["class_id"] != CLASS_IDS[label["root_cause"]]:
        raise ValueError(f"Target/root-cause mismatch in {record['sample_id']}")
    return {
        ...
        "messages": [
            *inference_messages(record),
            {"role": "assistant", "content": target_text(target)},
        ],
    }

...
from .data_io import ID_TO_LABEL, LABELS, resolve_path
...
def evidence_checks(
    ...
    acceptable_paths: set[str],
) -> list[dict[str, Any]]:
    checks = []
    for fact in response["evidence"]:
        exists = True
        try:
            actual = resolve_path(snapshot, fact["path"])
        ...
        checks.append(
            {
                ...
                "relevant": fact["path"] in acceptable_paths,
            }
        )
    return checks
...
def score_predictions(
    ...
    for sample_id, label in label_by_id.items():
        ...
        checks = evidence_checks(
            parsed,
            record_by_id[sample_id]["diagnostic_snapshot"],
            set(label["acceptable_evidence_paths"]),
        )
        fact_count += len(checks)
        grounded_facts += sum(item["grounded"] for item in checks)
        relevant_facts += sum(item["grounded"] and item["relevant"] for item in checks)
        relevant_cases += any(item["grounded"] and item["relevant"] for item in checks)
